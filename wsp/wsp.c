...
{
// UNUSED	static unsigned short prev_history_pos = -1;
	static char buf[32];
	char *b;
	int trycount = 0;
	weather_data_t d;
...
// UNUSED	prev_history_pos = history_pos;
...
void get_summary(weather_item_t *item, wsp_data* data)
{
	weather_data_t *wd = &item->data;
	int contact = has_contact_with_sensor(wd);

	data->itemp = wd->in_temp * 0.1;
	data->ihumi = wd->in_humidity;

   if(contact) 	{
   	// Only show current data if we have sensor contact

   	data->otemp = wd->out_temp * 0.1;
   	data->ohumi = wd->out_humidity;
   	data->opres = wd->abs_pressure * 0.1;
   	data->owspd = convert_avg_windspeed(wd);
   	data->owgus = convert_gust_windspeed(wd);
   	data->owdir = wd->wind_direction;
   	data->orain = wd->total_rain;
   	data->osens = 1;
	}
...
void print_summary(weather_settings_t *ws, weather_item_t *item)
{
	...
	if (contact)
	{
		...
		printf("  Total rain:\t\t%0.1f mm\n",			wd->total_rain * WS_RAIN_MM);
	}

	printf("\n");
}

void get_weather_data(struct usb_dev_handle *h,wsp_data* data)
{
	int i = 0;
	int history_address;
	...
	data->tstmp = history[HISTORY_MAX - 1].timestamp;
	get_summary(&history[HISTORY_MAX - 1],data);
...
void init_wsp_settings()
{
	...
	program_settings.debug = 0;
	program_settings.show_status = 0;
	program_settings.show_alarms = 0;
	program_settings.show_settings = 0;
	program_settings.show_current = 0;
	program_settings.show_maxmin = 0;
	program_settings.show_easyweather = 0;
	program_settings.show_summary = 0;
	program_settings.set_timezone = 0;
	program_settings.set_delay = 0;
	program_settings.altitude = 0;
	program_settings.show_formatted = 0;
	program_settings.show_formatlist = 0;
	program_settings.quickrain = 0;
	program_settings.dumpmem = 0;
	program_settings.from_file = 0;
	program_settings.f = 0;
}

void get_wsp_data(wsp_data* data)
{
   memset(data,1,sizeof(wsp_data));
	...
	program_settings.show_summary = 0;
	get_weather_data(devh,data);

	close_device(devh);
}

const char* time_string(struct tm* tm_timestamp)
{
   static char buffer[80];
   strftime(buffer,80,"%d-%b-%Y %X",tm_timestamp);
   return buffer;
}
...
void out_wsp_data(wsp_data* data)
{
   ...
   printf("\tUTC:  \t\t %s \n",time_string(gmtime(&pc_time)));
   printf("\tlocal:\t\t %s \n",time_string(localtime(&pc_time)));


   printf("\nLatest weather station data:\n");
   printf("\ttstmp:\t\t %d \t\ttimestamp\n",(unsigned int)data->tstmp);
   printf("\tUTC:  \t\t %s \tCoordinated Universal Time\n",time_string(gmtime(&data->tstmp)));
   printf("\tlocal:\t\t %s \tLocal Time\n",time_string(localtime(&data->tstmp)));
   printf("\titemp:\t\t %6.1f C   \t\tIndoor temperature\n",data->itemp);
   printf("\tihumi:\t\t %6.1f %%  \t\tIndoor humidity\n",data->ihumi);
   printf("\totemp:\t\t %6.1f C   \t\tOutdoor temperature\n",data->otemp);
   printf("\tohumi:\t\t %6.1f %%  \t\tOutdoor humidity\n",data->ohumi);
   printf("\topres:\t\t %6.1f hPa \t\tPressure\n",data->opres);
   printf("\towspd:\t\t %6.1f m/s \t\tAverage wind speed\n",data->owspd);
   printf("\towgus:\t\t %6.1f m/s \t\tGust wind speed\n",data->owgus);
   printf("\towdir:\t\t %6.1f deg \t\tWind direction in degrees, clockwise from North\n",data->owdir*22.5);
   printf("\torain:\t\t %6.1f mm  \t\tRain gauge\n",data->orain*0.3);
   printf("\tosens:\t\t %6s       \t\tOutside sensor contact\n", (data->osens)? "Ok" : "Error");
}
